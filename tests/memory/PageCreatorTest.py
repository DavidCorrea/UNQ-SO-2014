...
class PageCreatorTest(unittest.TestCase):
    ...
    def test_whenICreatePagesForPCB_ThenTheInfoHolderHasThemAsItShould(self):
        ...
        self.assertEqual(len(self.pcb.get_info_holder().get_pages()), 4)

    def test_whenICreatePagesForPCB_ThenTheInfoTheyHaveIsCorrect(self):
        self.page_creator.create(self.pcb, 5)
        page1 = self.pcb.get_info_holder().get_pages()[0]
        page2 = self.pcb.get_info_holder().get_pages()[1]
        page3 = self.pcb.get_info_holder().get_pages()[2]
        page4 = self.pcb.get_info_holder().get_pages()[3]
